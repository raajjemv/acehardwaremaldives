...
if ($_GET['action'] == "delete_confirm"){
	...
	if(mysql_query($sql)){
		$feedback_success = '<div class="alert alert-success"><i class="icon-ok-sign"></i>News / Event <strong>' . $_GET['name'] . '</strong> deleted from CMS.</div>';
		activityMonitor($global_user_id, $_GET['name'], "delete", "news-events");
	}else{
					...
					?>
				 	 <table class="table table-hover">
						...
						<tbody>
							...
							$sql = 'SELECT * FROM news WHERE title != "" ' . $sql_keyword . $sql_status . ' ORDER BY datetime DESC';
							if (!($result = mysql_query ($sql))){exit ('<b>Error:</b>' . mysql_error ());}
							
							$getTotal = mysql_num_rows($result);
							
							if($getTotal == 0){
								echo '<tr><td colspan="5"><div class="alert "><i class="icon-warning-sign"></i>No News / Event found.</div></td><tr>';
							}
							?>
							<?php 
							...
							<tr <?php if($i == 1){ echo 'class="first"'; } ?>>
								...
								<td><span class="label <?php echo $news_status_colors_array[$row['status']]; ?>"><?php echo $news_status_array[$row['status']]; ?></span></td>
								<td>
									<div class="btn-group pull-right">
										<a href="news.php?action=edit&id=<?php echo $row['id']; ?>"><button class="glow left"><i class="icon-pencil"></i></button></a>
