...
<?php
$main_page = 'store';
$sub_page  = 'brands.manager';
?>

<?php
if($_GET['action'] == "delete"){
	$delete_button_link = "brands.manager.php?action=delete_confirm&id=" . $_GET['id'] . "&name=" . $_GET['name'];
	$delete_confirm = '<div class="alert"><i class="icon-warning-sign pull-left"></i><a href="index.php" class="btn-flat white pull-right" style="margin-right: -20px; margin-top: 5px">Cancel</a> <a href="' . $delete_button_link . '" class="btn-flat white pull-right" style="margin-right: 10px; margin-top: 5px">Delete</a>  Are you sure you want to permanently delete the brand: <strong>' . $_GET['name'] .'</strong> . <br> You cant undo this action. </div>';
}

if ($_GET['action'] == "delete_confirm"){
	$get_id 	= sanitize($_GET['id'], INT);
	$sql = "DELETE FROM brand WHERE id = $get_id";
	if(mysql_query($sql)){
		$feedback_success = '<div class="alert alert-success"><i class="icon-ok-sign"></i>Brand <strong>' . $_GET['name'] . '</strong> deleted from the online store.</div>';
		activityMonitor($global_user_id, $_GET['name'], "delete", "store-brands");
	}else{
		 	...
		 	<div class="table-wrapper products-table " style="margin-bottom: 0;">	
				...
				<div class="row-fluid filter-block">
					<div class="pull-right">
						<form method="post" action="brands.manager.php">
			 	 ...
			 	 <div class="row-fluid">
					...
					?>
				 	 <table class="table table-hover">
						...
						<tbody>
							...
							if($filter_keyword){ $sql_keyword = 'AND name LIKE "%' . $filter_keyword . '%" OR name LIKE "%' . $filter_keyword . '%"'; }
							if($filter_status){ $sql_status = " AND status = $filter_status"; }
							
							$sql = 'SELECT * FROM brand WHERE name != "" ' . $sql_keyword . $sql_status . ' ORDER BY name ASC LIMIT ' . $offset .', ' . $limit;
							//echo $sql;
							if (!($result = mysql_query ($sql))){exit ('<b>Error:</b>' . mysql_error ());}
							
							$sql2 = 'SELECT * FROM brand WHERE name != "" ' . $sql_keyword . $sql_status . ' ';
							...
							if($getTotal == 0){
								echo '<tr><td colspan="5"><div class="alert "><i class="icon-warning-sign"></i>No brands found in the store.</div></td><tr>';
							}
							?>
							<?php 
							...
							<tr <?php if($i == 1){ echo 'class="first"'; } ?>>
								...
								<td>
									<div class="btn-group pull-right">
										<a href="brands.php?action=edit&id=<?php echo $row['id']; ?>"><button class="glow left"><i class="icon-pencil"></i></button></a>
										<a href="brands.manager.php?action=delete&id=<?php echo $row['id']; ?>&name=<?php echo $row['name']; ?>"><button class="glow right"><i class="icon-remove"></i></button></a>
									</div>
								</td>
							</tr>
							<?php } ?>
					...
					<div class="pagination" style="margin: 0 auto; text-align: center; border-top: 1px solid #EEE; border-bottom: 1px solid #EEE; padding: 15px 0; background-color: #FDFDFD;">
						...
						<ul>
							...
							<li><a href="brands.manager.php?page=<?php echo $current_page-1; ?>&keyword=<?php echo $filter_keyword; ?>&parent_category=<?php echo $filter_parent_category; ?>&status=<?php echo $filter_status; ?>">&#8249;</a></li>
							<?php }else{ ?>
							<li><a href="#">&#8249;</a></li>
							<?php } ?>
							
						  	<?php for ($i = 1; $i < $totalPages; $i++) { ?>
						  	<li class="<?php echo ($page == $i) ? "active" : "" ;?>"><a href="brands.manager.php?page=<?php echo $i; ?>&keyword=<?php echo $filter_keyword; ?>&parent_category=<?php echo $filter_parent_category; ?>&status=<?php echo $filter_status; ?>"><?php echo $i; ?></a></li>
						  	<?php } ?>
