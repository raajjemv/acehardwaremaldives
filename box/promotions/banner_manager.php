...
$sub_page  = 'banner.manager';
?>

<?php
if($_GET['action'] == "delete"){
	$delete_button_link = "banner_manager.php?action=delete_confirm&id=" . $_GET['id'] . "&name=" . $_GET['name'];
	$delete_confirm = '<div class="alert"><i class="icon-warning-sign pull-left"></i><a href="banner_manager.php" class="btn-flat white pull-right" style="margin-right: -20px; margin-top: 5px">Cancel</a> <a href="' . $delete_button_link . '" class="btn-flat white pull-right" style="margin-right: 10px; margin-top: 5px">Delete</a>  Are you sure you want to permanently delete the banner: <strong>' . $_GET['name'] .'</strong> . <br> You cant undo this action. </div>';
}
...
if ($_GET['action'] == "delete_confirm"){
	...
	if(mysql_query($sql)){
		$feedback_success = '<div class="alert alert-success"><i class="icon-ok-sign"></i>Banner <strong>' . $_GET['name'] . '</strong> deleted from the system.</div>';
					...
					?>
				 	 <table class="table table-hover">
						...
						<tbody>
							...
							//
							if($filter_keyword){ $sql_keyword = " AND title LIKE '%$filter_keyword%' OR descr = '$filter_keyword'"; }
							if($filter_status){ $sql_status = " AND status = $filter_status"; }
							
							$sql = 'SELECT * FROM banner WHERE id != "" ' . $sql_keyword . $sql_status . ' ORDER BY id DESC';
							...
							<tr <?php if($i == 1){ echo 'class="first"'; } ?>>
								...
								<td>
									<div class="btn-group pull-right">
										<a href="banner.php?action=edit&id=<?php echo $row['id']; ?>"><button class="glow left"><i class="icon-pencil"></i></button></a>
										<a href="banner_manager.php?action=delete&id=<?php echo $row['id']; ?>&name=<?php echo $row['title']; ?>"><button class="glow right"><i class="icon-remove"></i></button></a>
									</div>
								</td>
							</tr>
							<?php } ?>
